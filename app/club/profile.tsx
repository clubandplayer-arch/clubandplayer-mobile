...
const CLUB_FIELDS: FieldConfig[] = [
  {
    key: "club_foundation_year",
    label: "Anno di fondazione",
    keyboardType: "numeric",
  },
  { key: "club_stadium", label: "Stadio" },
  { key: "club_stadium_address", label: "Indirizzo stadio" },
  { key: "club_league_category", label: "Categoria lega" },
  { key: "club_motto", label: "Motto" },
];

const FORM_FIELDS: FieldConfig[] = [...COMMON_FIELDS, ...CLUB_FIELDS];
...
export default function ClubProfileScreen() {
  ...
  const [form, setForm] = useState<Record<ProfilePatchField, string>>(() =>
    buildInitialForm(null, FORM_FIELDS),
  );
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setProfile(payload);
      setForm(buildInitialForm(payload, FORM_FIELDS));
      setLoading(false);
    };
  ...
  const onSave = async () => {
    ...
    const payload = buildPayload(form, FORM_FIELDS);
    ...
    if (refreshed.ok) {
      ...
      setForm(buildInitialForm(nextProfile, FORM_FIELDS));
    }
