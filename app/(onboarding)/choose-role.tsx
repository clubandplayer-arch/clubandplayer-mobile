...
function getTargetRoute(accountType: string | null | undefined) {
  return resolvePostAuthPath({ account_type: accountType });
}
  ...
  useEffect(() => {
    ...
    const loadProfile = async () => {
      ...
      if (!response.ok) {
        Alert.alert(
          "Errore",
          response.errorText ?? "Impossibile leggere il profilo",
        );
        setLoading(false);
        return;
      }
  ...
  const selectRole = async (accountType: AccountType) => {
    try {
      ...
      if (!response.ok) {
        Alert.alert(
          "Errore",
          response.errorText ?? "Impossibile salvare il ruolo",
        );
        return;
      }
  ...
  return (
    ...
    >
      ...
      <Text style={{ fontSize: 16, color: theme.colors.muted, lineHeight: 22 }}>
        Seleziona come vuoi usare Club & Player. Potrai completare il resto del
        profilo nel passaggio successivo.
      </Text>
